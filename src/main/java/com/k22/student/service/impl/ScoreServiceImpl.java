...
import com.k22.student.service.ScoreServiceInterface;
...
public class ScoreServiceImpl implements ScoreServiceInterface {
    ...
    public ArrayList<Score> getList(int studentId) {
        ...
        try {
            PreparedStatement statement = jdbcConnect.connection().prepareStatement(query);
    ...
    public ArrayList<TypeScore> getListTypeScore() {
        try {
            PreparedStatement statement = jdbcConnect.connection().prepareStatement("select * from type_score");
    ...
    public void saveScoreDetail(int scid, int sid) {
        try {
            PreparedStatement preparedStatement = jdbcConnect.connection().prepareStatement("insert into score_details(sid, scid) \n" +
                    "values (?,?)");
    ...
    public Score findScoreById(int id) {
        try {
            PreparedStatement statement = jdbcConnect.connection().prepareStatement("SELECT * from scores where scid = ?");
